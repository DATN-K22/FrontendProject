...
export default function LabDetail() {
  ...
  useEffect(() => {
    const fetchLabData = async () => {
      try {
        const response = await api.get(`/courses/lessons/${lab_id}/${userData?.id}`)
  ...
  // Fetch lease with polling until Active
  useEffect(() => {
  ...
  // Countdown timer
  useEffect(() => {
    if (timeLeft <= 0 || leaseData?.status !== 'Active') {
  ...
  const handleConfirmEndLab = async () => {
    ...
    setEndingLab(true)
    try {
      await api.post(`/courses/lab/leases/${leaseId}/terminate`, {
        userId: userData?.id,
        chapterItemId: lab_id
      })
      if (pollingRef.current) clearInterval(pollingRef.current)
      if (countdownRef.current) clearInterval(countdownRef.current)
    } catch (error) {
  ...
  if (loadingLab) {
    return (
      <Box sx={{ minHeight: '100vh', background: 'linear-gradient(135deg, #e3f2fd 0%, #f5f5f5 100%)' }}>
  ...
  if (!labData) {
    return (
      <Box
        sx={{ minHeight: '100vh', bgcolor: '#f5f7fa', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      >
  ...
  return (
    <Box sx={{ minHeight: '100vh', background: 'linear-gradient(135deg, #e3f2fd 0%, #f5f5f5 100%)', pb: 8 }}>
        ...
        <Box sx={{ p: 3 }}>
          ...
          <Box sx={{ mb: 3 }}>
            <Typography
              variant='h4'
              component='h1'
              sx={{ fontWeight: 'bold', color: 'text.primary', fontSize: { xs: '1.75rem', md: '2.125rem' } }}
            >
              {labData.title}
            </Typography>
          </Box>
          ...
          {/* Main Layout */}
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '5fr 2fr' }, gap: 3 }}>
            {/* Left column */}
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
              {/* Timer + End Lab bar */}
              <Paper
              ...
              >
                ...
                <Button
                  ...
                  sx={{
                    ...
                    '&.Mui-disabled': { background: '#e0e0e0', color: 'rgba(0,0,0,0.45)' }
                  }}
                >
            ...
            {/* Sidebar */}
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
              <Paper elevation={0} sx={{ p: 3, borderRadius: '20px', border: '1px solid', borderColor: 'grey.200' }}>
                ...
                <Box sx={{ mb: 2 }}>
                  ...
                  >
                    {loadingConsoleUrl ? 'Loading…' : 'Go to console'}
                  </Button>
                </Box>
              </Paper>
            </Box>
      ...
      {/* Confirm End Lab dialog */}
      <Dialog open={endConfirmOpen} onClose={() => !endingLab && setEndConfirmOpen(false)} fullWidth maxWidth='xs'>
        <DialogTitle sx={{ fontWeight: 700, color: '#5d4700' }}>End Lab</DialogTitle>
        <DialogContent>
          <Typography variant='body2' color='text.secondary' sx={{ mt: 1 }}>
            Are you sure you want to end this lab session? This action cannot be undone.
          </Typography>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => setEndConfirmOpen(false)} disabled={endingLab} sx={{ color: 'grey' }}>
            Cancel
          </Button>
          <Button
            variant='contained'
