...
const expandRecurringStarts = (startAt: Date, rrule: string, rangeStart: Date, rangeEnd: Date): Date[] => {
...
const extractEventsFromPayload = (payload: unknown): BackendEvent[] => {
  ...
  const directCandidates = [maybeObject.data, maybeObject.events, maybeObject.items, maybeObject.results]
...
export default function SchedulePage() {
  ...
  const [displayOffsetMinutes, setDisplayOffsetMinutes] = useState(
    () => defaultTimezone?.offsetMinutes ?? detectedOffset
  )
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      try {
        ...
        const dedupedById = Array.from(new Map(mergedEvents.map((event) => [String(event.id), event])).values())
  ...
  const executeSave = async (scope: 'this' | 'thisAndFollowing' | 'all') => {
    ...
    try {
      if (!selectedEvent) {
        ...
        await api.post(`/users/schedule/events/${masterEventId}/split`, {
          // ← masterEventId
          recurrence_id: occurrenceIso,
          updates: payload
        })
      ...
      const message =
        error?.response?.data?.message || (selectedEvent ? 'Failed to update event' : 'Failed to create event')
  ...
  const executeDelete = async (scope: 'this' | 'thisAndFollowing' | 'all') => {
    ...
    try {
      if (scope === 'this') {
        await api.post(`/users/schedule/events/${masterEventId}/exceptions`, {
          // ← masterEventId
        ...
        await api.put(`/users/schedule/events/${masterEventId}`, {
          ...
          time_start:
            masterEvent.time_start instanceof Date ? masterEvent.time_start.toISOString() : masterEvent.time_start,
          time_end: masterEvent.time_end instanceof Date ? masterEvent.time_end.toISOString() : masterEvent.time_end,
        ...
        >
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '240px 1fr 300px' } }}>
                        ...
                        {group.events.map((event) => (
                          <ListItem key={event.id} disableGutters sx={{ py: 0.4, px: 0 }}>
                            <Stack spacing={0.25} sx={{ width: '100%' }}>
                              ...
                              <Typography sx={{ fontSize: 11, color: '#4A3A12', pl: 2.2 }}>{event.title}</Typography>
                            </Stack>
                          </ListItem>
                        ))}
                      </Box>
            ...
            <Box sx={{ borderLeft: { lg: '1px solid #F1E4BA' }, backgroundColor: '#FFFDF3', p: 2 }}>
              <Paper variant='outlined' sx={{ p: 1.5, borderColor: '#EEDFAE', backgroundColor: '#FFF9DF' }}>
                ...
                <Stack spacing={1.25}>
                  ...
                  <Typography sx={{ fontSize: 11, color: '#8A7640' }}>{formTimezoneLabel}</Typography>
