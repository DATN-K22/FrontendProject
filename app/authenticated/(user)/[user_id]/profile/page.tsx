...
// ─── Icons ─────────────────────────────────────────────────────────────────────
export const EyeIcon = ({ off }: { off?: boolean }) => (off ? <VisibilityOffIcon /> : <VisibilityIcon />)
...
function AvatarDialog({
  ...
  return (
    <Dialog open={open} onClose={handleClose} PaperProps={{ sx: { borderRadius: 4, p: 1, minWidth: 360 } }}>
      <DialogTitle sx={{ fontWeight: 600, fontSize: '1.2rem', textAlign: 'center' }}>Update Avatar</DialogTitle>
      <DialogContent>
        <Box
          sx={{
            display: 'flex',
        ...
        >
          ...
          <Button variant='outlined' onClick={() => fileRef.current?.click()} sx={ghostBtn} size='small'>
            Choose Image
          </Button>
          {error && (
          ...
          <Box sx={{ display: 'flex', gap: 2, width: '100%' }}>
            <Button fullWidth variant='outlined' onClick={handleClose} sx={ghostBtn}>
              Cancel
            </Button>
            <Button fullWidth variant='contained' onClick={handleUpload} disabled={!file || loading} sx={primaryBtn}>
              {loading ? <CircularProgress size={20} sx={{ color: C.ink }} /> : 'Save'}
            </Button>
          </Box>
        </Box>
      </DialogContent>
...
function EditProfileDialog({
  ...
  return (
    <Dialog open={open} onClose={handleClose} PaperProps={{ sx: { borderRadius: 4, p: 1, minWidth: 400 } }}>
      <DialogTitle sx={{ fontWeight: 600, fontSize: '1.2rem', textAlign: 'center', pb: 0 }}>
        ...
        <Box sx={{ pt: 1 }}>
          {saved ? (
            ...
            >
              ...
              >
                <svg width='40' height='40' viewBox='0 0 24 24' fill='none' stroke={C.yellowDark} strokeWidth='2'>
              ...
              <Button fullWidth variant='contained' onClick={handleClose} sx={primaryBtn}>
                Close
              </Button>
            </Box>
          ) : (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
              ...
              <Box sx={{ gap: 2, mt: 1 }}>
                <Button
                  ...
                  sx={{ ...primaryBtn, mb: 1 }}
                >
                  {loading ? <CircularProgress size={20} sx={{ color: C.ink }} /> : 'Save Changes'}
                </Button>
                <Button fullWidth variant='outlined' onClick={handleClose} sx={ghostBtn}>
                  Cancel
                </Button>
...
function ChangePasswordDialog({ open, onClose, userId }: ChangePasswordDialogProps) {
  ...
  const handleSubmitPw = async () => {
    ...
    if (!confirmPw) {
      ...
      newErrors.confirm_password = 'New password and confirmation do not match.'
    }
    ...
    try {
      ...
      setError('Fail to update password. Please check your current password and try again.')
    } finally {
      setLoading(false)
    }
  }
          ...
          {step === 'form' && (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                    ...
                    InputProps={{
                      endAdornment: (
                        <InputAdornment position='end'>
                          <IconButton onClick={() => field.setShow(!field.show)} size='small'>
              ...
              <Button
                ...
                sx={{ ...primaryBtn, mt: 1 }}
              >
                {loading ? <CircularProgress size={20} sx={{ color: C.ink }} /> : 'Change Password'}
              </Button>
              <Button fullWidth variant='outlined' onClick={handleClose} sx={ghostBtn}>
                Cancel
              </Button>
...
export default function Profile() {
  ...
  const handleProfileSuccess = (firstName: string, lastName: string) => {
    setUser((u) => (u ? { ...u, first_name: firstName, last_name: lastName } : u))
  ...
  return (
    <>
          ...
          >
            ...
            >
              ...
              >
                ...
                <Box sx={{ flex: 1 }}>
                  <Typography sx={{ fontWeight: 600, fontSize: '0.95rem' }}>Change Password</Typography>
      ...
      <ChangePasswordDialog open={pwOpen} onClose={() => setPwOpen(false)} userId={user.id} />
    </>
  )
}
