...
export default function QuizStart() {
  ...
  const [feedbackQuestion, setFeedbackQuestion] = useState<QuizQuestion | null>(
    null,
  );
  const [pendingNextQuestion, setPendingNextQuestion] =
    useState<QuizQuestion | null>(null);
  const [pendingSummary, setPendingSummary] = useState<QuizSummary | null>(
    null,
  );

  const apiRoot = useMemo(() => {
    if (!userData?.id || !quizId) return null;
    return `/courses/quizzes/${userData.id}/${quizId}`;
  }, [quizId, userData?.id]);
  ...
  const loadQuiz = useCallback(async () => {
    ...
    try {
      const response = await api.post(apiRoot);
      const payload = (response.data?.data ??
        response.data) as TakeQuizResponse;
      ...
      if (payload.type === "summary") {
        setSummary({
          ...
          encouragement:
            payload.encouragement ?? "Try again after reviewing the material.",
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      ...
      const payload = (response.data?.data ??
        response.data) as SubmitAnswerResponse;
      ...
      if (
        payload.type !== "feedback_final" &&
        payload.type !== "feedback_with_next" &&
        !payload.nextQuestion
      ) {
        showAlert(
          "Your answer was saved. Continue when you are ready.",
          "info",
          ...
          >
            <Stack spacing={3} alignItems="flex-start">
              ...
              <Box>
                <Typography
                  variant="h4"
                  sx={{ fontWeight: 800, color: QUIZ_COLORS.text, mb: 1 }}
                >
                  Quiz completed
                </Typography>
              </Box>
              ...
              >
                ...
                >
                  <Typography
                    variant="body2"
                    sx={{ color: QUIZ_COLORS.muted, mb: 0.5 }}
                  >
                    Accuracy
                  </Typography>
                  <Typography
              ...
              >
                <Typography
                  variant="h6"
                  sx={{ fontWeight: 800, mb: 1.5, color: QUIZ_COLORS.text }}
                >
                  System feedback
                </Typography>
            ...
            <Stack spacing={3}>
              ...
              >
                <Box>
                  <Typography
                    variant="overline"
                    sx={{ color: QUIZ_COLORS.primaryDeep, fontWeight: 800 }}
                  >
                    Quiz Session
                  </Typography>
                  <Typography
                ...
                {progress && (
                  <Stack spacing={1} sx={{ minWidth: { xs: "100%", md: 320 } }}>
                    <Stack direction="row" justifyContent="space-between">
                      <Typography
                        variant="body2"
                        sx={{ color: QUIZ_COLORS.muted, fontWeight: 700 }}
                      >
              ...
              {question ? (
                <Stack spacing={2.5}>
                  <Box>
                    <Chip
                      label={question.questionType ?? "Multiple choice"}
                      sx={accentChipSx}
                    />
                  </Box>
                  <Typography
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            <Typography
                              sx={{ lineHeight: 1.7, color: QUIZ_COLORS.text }}
                            >
                              {option.optionText}
                            </Typography>
                          </Stack>
                  ...
                  {hasFeedback && feedbackQuestion && (
                    ...
                    >
                      <Stack spacing={1.5}>
                        ...
                        <Typography
                          sx={{ color: QUIZ_COLORS.muted, lineHeight: 1.75 }}
                        >
                        ...
                        <Typography
                          variant="body2"
                          sx={{ fontWeight: 700, color: QUIZ_COLORS.text }}
                        >
                          Option explanations:
                        </Typography>
                        <Stack spacing={1}>
                          {feedback?.optionReasons?.length ? (
                            feedback.optionReasons.map((item) => {
                              const optionIndex =
                                feedbackQuestion.options.findIndex(
                ...
                >
                  <Stack spacing={1.5} alignItems="flex-start">
                    ...
                    <Typography sx={{ color: QUIZ_COLORS.muted }}>
                      If the quiz is complete, you can review the results on the
                      overview page.
                    </Typography>
                    <Button
                      variant="contained"
