...
export default function LabOverview() {
  ...
  const fetchLabData = async () => {
    try {
      const lessonResponse = await api.get(`/lessons/${lab_id}/${userData?.id}`)
  ...
  const handleStartLab = async () => {
    ...
    try {
      ...
      if (!activeSession) {
        ...
        router.push(`/authenticated/course/${course_id}/lab/${lab_id}/start?leaseId=${leaseSession.leaseId}`)
      } else {
        router.push(`/authenticated/course/${course_id}/lab/${lab_id}/start?leaseId=${activeSession.leaseId}`)
      }
    } catch (err) {
